...
ProcessTelemetry collect_process_telemetry(pid_t pid) {
  ...
  if (!stat_content.empty()) {
    ...
    if (close_paren != std::string::npos &&
        close_paren + 2 < stat_content.length()) {
      std::string rest = stat_content.substr(close_paren + 2);
      std::vector<std::string> fields = split_whitespace(rest);

      // Offset fields since we skipped the PID and COMM name (which are fields
      // 1 and 2) field index in rest is (original index - 3)

      if (fields.size() > 0)
        pt.state = fields[0];
      if (fields.size() > 1)
        pt.ppid = safe_stoi(fields[1]);
      if (fields.size() > 7)
        pt.minflt = safe_stoull(fields[7]);
      if (fields.size() > 9)
        pt.majflt = safe_stoull(fields[9]);
      if (fields.size() > 11)
        pt.utime = safe_stoull(fields[11]);
      if (fields.size() > 12)
        pt.stime = safe_stoull(fields[12]);
      if (fields.size() > 17)
        pt.thread_count = safe_stoi(fields[17]);
      if (fields.size() > 20)
        pt.vsize_bytes = safe_stoull(fields[20]);
      if (fields.size() > 21) {
  ...
  if (status_file.is_open()) {
    ...
    while (std::getline(status_file, line)) {
      if (utils::starts_with(line, "voluntary_ctxt_switches:")) {
        ...
        if (parts.size() > 1)
          pt.voluntary_ctxt_switches = safe_stoull(utils::trim(parts[1]));
      } else if (utils::starts_with(line, "nonvoluntary_ctxt_switches:")) {
        std::vector<std::string> parts = utils::split(line, ':');
        if (parts.size() > 1)
          pt.involuntary_ctxt_switches = safe_stoull(utils::trim(parts[1]));
      }
    }
  }

  // 4. Read /proc/[pid]/schedstat
  std::string schedstat_content =
      utils::read_file_content(pid_dir + "/schedstat");
    ...
    while (std::getline(mem_file, line)) {
      ...
      if (parts.size() > 1) {
        ...
        if (key == "MemTotal")
          ...
          st.mem_available_kb = val;
        else if (key == "Cached")
          st.mem_cached_kb = val;
        else if (key == "Buffers")
          st.mem_buffers_kb = val;
      }
    }
  }
...
std::string serialize_telemetry_to_json_string(const ProcessTelemetry &pt,
  ...
  if (pt.env.size() > 30) {
    jenv.push_back("... (truncated " + std::to_string(pt.env.size() - 30) +
...
std::string collect_ebpf_telemetry(pid_t pid, int duration_seconds) {
  ...
  if (!has_privileges) {
    std::cerr << "\n⚠️  eBPF tracing requires root privileges. Please run "
                 "SysPilot with sudo:"
              << std::endl;
    std::cerr << "   sudo ./syspilot explain --pid " << pid << " --ebpf"
              << std::endl;
    return "Error: Insufficient privileges to run eBPF bpftrace.";
  }

  std::cout << "🚀 Starting real-time eBPF event tracing on PID " << pid << " ("
            << duration_seconds << "s)..." << std::endl;

  std::string filter =
      "pid == " + std::to_string(pid) + " || ppid == " + std::to_string(pid);
  ...
  std::string summary = "=== eBPF Kernel Event Log (Duration: " +
