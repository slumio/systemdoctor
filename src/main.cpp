...
std::vector<LogEntry> read_recent_entries(size_t count) {
    ...
    if (!utils::file_exists(path)) return entries;
    ...
    for (auto it = lines.rbegin(); it != lines.rend(); ++it) {
        ...
        if (parts.size() == 4) {
            ...
            if (utils::starts_with(cmd, "syspilot")) continue;
...
std::string tail_session(size_t max_lines) {
    ...
    if (!utils::file_exists(path)) return "";
...
void print_help() {
    std::cout << "🤖 SysPilot: Operating System Reasoning Agent (C++ Edition)\n\n"
              ...
              << "     --ebpf                     Enable real-time eBPF event tracing (requires root/sudo)\n"
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "install") {
        ...
        if (argc < 3) {
            std::cerr << "❌ Expected provider name (gemini, ollama, syspilot)" << std::endl;
            return 1;
        }
        std::string prov = utils::to_lower(argv[2]);
        if (prov != "gemini" && prov != "ollama" && prov != "syspilot") {
            std::cerr << "❌ Unknown provider: " << prov << ". Use: gemini, ollama, syspilot." << std::endl;
            return 1;
        }
        ...
        if (conf.active_provider == "gemini") conf.gemini_model = model;
        else if (conf.active_provider == "ollama") conf.ollama_model = model;
        else if (conf.active_provider == "syspilot") conf.syspilot_model = model;
        
        config::save(conf);
        std::cout << "✅ Model set to " << model << " for " << conf.active_provider << std::endl;
    } 
    else if (cmd == "pull") {
        if (argc < 3) {
            std::cerr << "❌ Expected model name." << std::endl;
        ...
        if (conf.active_provider != "ollama") {
            std::cout << "⚠️ Current provider is " << conf.active_provider << ". Model pulling only works with Ollama." << std::endl;
        ...
        if (argc < 3) {
            std::cerr << "❌ Expected config action (set-key, set-url, set)" << std::endl;
            return 1;
        }
        std::string action = argv[2];
        if (action == "set-key") {
            if (argc < 5) {
                std::cerr << "❌ Expected: syspilot config set-key <provider> <key>" << std::endl;
                return 1;
            }
            std::string prov = utils::to_lower(argv[3]);
            std::string key = argv[4];
            if (prov == "gemini") conf.gemini_api_key = key;
            else if (prov == "syspilot") conf.syspilot_api_key = key;
            else {
                std::cerr << "❌ Provider " << prov << " does not use API keys." << std::endl;
                return 1;
            }
            config::save(conf);
            ...
            if (argc < 5) {
                std::cerr << "❌ Expected: syspilot config set-url <provider> <url>" << std::endl;
                return 1;
            }
            std::string prov = utils::to_lower(argv[3]);
            std::string url = argv[4];
            if (prov == "ollama") conf.ollama_url = url;
            else {
                std::cerr << "❌ Provider " << prov << " does not support custom URLs." << std::endl;
                return 1;
            }
            config::save(conf);
            ...
            if (argc < 5) {
                std::cerr << "❌ Expected: syspilot config set <option> <value>" << std::endl;
                return 1;
            }
            ...
            if (option == "chunk_strategy" || option == "strategy") {
                if (val != "syntactic" && val != "line") {
                    std::cerr << "❌ Invalid chunk strategy: " << val << ". Options: syntactic, line" << std::endl;
                    return 1;
                }
                ...
                std::cerr << "❌ Unknown option: " << option << ". Options: chunk_strategy, embedding_model" << std::endl;
                return 1;
            }
            config::save(conf);
        ...
        ctx["file_list"] = utils::trim(utils::run_command_output("ls -lA 2>/dev/null"));
        
        if (utils::file_exists(".git")) {
            ctx["git_branch"] = utils::trim(utils::run_command_output("git branch --show-current 2>/dev/null"));
            ctx["git_status"] = utils::trim(utils::run_command_output("git status --porcelain 2>/dev/null"));
        }
        ...
        if (!file_path.empty()) {
            ...
            if (ok) {
                ...
                ctx["file_content"] = "File '" + file_path + "' contents:\n" + truncated;
            } else {
        ...
        if (!no_index) {
            ...
            ctx["codebase_context"] = codebase::query_context(ctx["current_dir"], question, conf);
        }
        
        std::string prompt = "Terminal Context:\n" + ctx.dump(4) + "\n\nQuestion: " + question;
        ...
        if (!target_pid_or_name.empty()) {
            ...
            if (pid == 0) {
                std::cerr << "❌ Could not find process PID for: " << target_pid_or_name << std::endl;
                return 1;
            }
            ...
            if (causal) {
                ...
                if (!no_index) {
                    ...
                    ctx["codebase_context"] = codebase::query_context(ctx["current_dir"], target_pid_or_name, conf);
                }
            } else {
                // ==========================================
                // 1. New Operating System Reasoning Agent Mode
                // ==========================================
                std::cout << "📊 Gathering low-level telemetry for PID " << pid << " (" << target_pid_or_name << ")..." << std::endl;
                ...
                std::cout << "🔬 Extracting thread stack traces & active profile..." << std::endl;
                ProfileReport pr = profiler::profile_process(pid, deep); // Run perf hotspot profiler if deep
                
                ctx["telemetry"] = json::parse(telemetry::serialize_telemetry_to_json_string(pt, st));
                ctx["execution_profile"] = json::parse(profiler::serialize_profile_to_json_string(pr));
                ...
                for (const auto& stack : pr.active_stacks) {
                    for (size_t i = 0; i < std::min(stack.frames.size(), (size_t)3); ++i) {
                        query_terms += " " + stack.frames[i];
                    }
                }
                
                if (!no_index) {
                    std::cout << "🔍 Mapping execution profile back to codebase..." << std::endl;
                    ctx["codebase_context"] = codebase::query_context(ctx["current_dir"], query_terms, conf);
                }
            ...
            if (deep) {
                ctx["disk_usage"] = utils::trim(utils::run_command_output("df -h 2>/dev/null"));
                ctx["memory_usage"] = utils::trim(utils::run_command_output("free -h 2>/dev/null"));
                ctx["open_ports"] = utils::trim(utils::run_command_output("ss -tlnp 2>/dev/null"));
            }
            
            if (!no_index) {
                std::cout << "🔍 Searching codebase for command context..." << std::endl;
                std::string fallback_query = target_entry.command + "\n" + tail_session(10);
                ctx["codebase_context"] = codebase::query_context(ctx["current_dir"], fallback_query, conf);
            }
        ...
        std::string prompt;
        if (causal) {
            prompt = "You are a senior system reliability engineer performing root-cause analysis.\n"
                     "We have built a directed dependency graph of the operating system processes and resources in real-time.\n"
                     "When a performance anomaly was detected or requested, we performed a reverse Breadth-First Search (BFS) "
                     "starting from the symptomatic process node to trace back to potential root causes.\n\n"
                     "Here is the structured JSON representation of the traversed causal path (nodes and edges):\n"
                     + ctx.dump(4) + "\n\n"
                     "Please explain the diagnostic findings to the user:\n"
                     "1. User-facing symptom (the starting process and what is anomalous about it)\n"
                     "2. Step-by-step root cause chain (how the symptom maps back to the resources, blockages, or other processes)\n"
                     "3. Which specific process is the root cause, and why\n"
                     "4. Recommended actionable mitigation (e.g. kill -STOP <PID>, optimize disk access, scale resources)\n"
                     "Be extremely specific, detail I/O rates or CPU usage where applicable. Provide a Confidence Score (0-100%).";
        } else {
            prompt = "Perform causal reasoning on the following OS telemetry and execution context to diagnose the system state.\n\n"
