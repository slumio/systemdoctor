...
std::string CausalGraph::export_graph_to_html(const std::vector<std::string>& path_nodes) {
    ...
    for (const auto& node_id : path_nodes) {
        ...
        if (it != nodes.end()) {
            ...
            n["data"]["type"] = (it->second.type == NodeType::PROCESS) ? "process" : "resource";
    ...
    oss << "    <title>SysPilot CausalTrace - Interactive Dependency Graph</title>\n";
    oss << "    <link rel=\"preconnect\" href=\"https://fonts.googleapis.com\">\n";
    oss << "    <link rel=\"preconnect\" href=\"https://fonts.gstatic.com\" crossorigin>\n";
    oss << "    <link href=\"https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;800&family=JetBrains+Mono:wght@400;700&display=swap\" rel=\"stylesheet\">\n";
    oss << "    <script src=\"https://cdnjs.cloudflare.com/ajax/libs/cytoscape/3.26.0/cytoscape.min.js\"></script>\n";
    oss << "    <script src=\"https://cdn.jsdelivr.net/npm/dagre@0.8.5/dist/dagre.min.js\"></script>\n";
    oss << "    <script src=\"https://cdn.jsdelivr.net/npm/cytoscape-dagre@2.5.0/cytoscape-dagre.js\"></script>\n";
    oss << "    <script src=\"https://cdnjs.cloudflare.com/ajax/libs/d3/7.8.5/d3.min.js\"></script>\n";
    oss << "    <style>\n";
    oss << "        :root {\n";
    oss << "            --bg-color: #0b0f17;\n";
    ...
    oss << "            background: radial-gradient(circle at center, #0f172a 0%, #020617 100%);\n";
    oss << "            z-index: 100;\n";
    oss << "            display: flex;\n";
    oss << "            flex-direction: column;\n";
    ...
    oss << "            background: linear-gradient(135deg, #38bdf8 0%, #0284c7 50%, #f97316 100%);\n";
    ...
    oss << "        }\n";
    oss << "        .enter-btn {\n";
    oss << "            background: linear-gradient(135deg, #0284c7 0%, #0369a1 100%);\n";
    ...
    oss << "            background: linear-gradient(135deg, #0369a1 0%, #0284c7 100%);\n";
    oss << "        }\n";
    oss << "        header {\n";
    oss << "            display: flex;\n";
    ...
    oss << "            margin: 0;\n";
    oss << "            font-weight: 800;\n";
    oss << "            background: linear-gradient(135deg, #38bdf8 0%, #0284c7 100%);\n";
    oss << "            -webkit-background-clip: text;\n";
    oss << "            -webkit-text-fill-color: transparent;\n";
    oss << "            display: flex;\n";
    oss << "            align-items: center;\n";
    ...
    oss << "            flex: 1;\n";
    oss << "            height: 100%;\n";
    oss << "            background: radial-gradient(circle at 50% 50%, #111827 0%, #0b0f17 100%);\n";
    oss << "        }\n";
    oss << "        #d3-container {\n";
    oss << "            display: none;\n";
    oss << "        }\n";
    ...
    oss << "            <div class=\"intro-subtitle\">Automated Root-Cause Diagnostic Report</div>\n";
    oss << "            <div class=\"console-box\" id=\"intro-console\"></div>\n";
    oss << "            <button class=\"enter-btn\" onclick=\"dismissIntro()\">EXPLORE CAUSAL DEPENDENCIES</button>\n";
    oss << "        </div>\n";
    oss << "    </div>\n";
    oss << "    <header>\n";
    oss << "        <div>\n";
    oss << "            <h1>🌐 SysPilot CausalTrace</h1>\n";
    oss << "            <div class=\"subtitle\">Interactive Root Cause Diagnostics</div>\n";
    oss << "        </div>\n";
    oss << "        <div class=\"view-selector\">\n";
    oss << "            <button class=\"view-btn active\" id=\"btn-cytoscape\" onclick=\"switchView('cytoscape')\">Cytoscape View</button>\n";
    oss << "            <button class=\"view-btn\" id=\"btn-d3\" onclick=\"switchView('d3')\">D3.js View</button>\n";
    oss << "        </div>\n";
    ...
    oss << "            <button class=\"control-btn\" onclick=\"cy.fit()\">Fit Screen</button>\n";
    oss << "            <button class=\"control-btn\" onclick=\"resetHighlight()\">Reset Focus</button>\n";
    oss << "            <button class=\"control-btn\" id=\"toggle-layout-btn\" onclick=\"toggleLayout()\">Layout: DAGRE</button>\n";
    oss << "        </div>\n";
    oss << "        \n";
    oss << "        <div id=\"cy\"></div>\n";
    ...
    oss << "                <div class=\"legend-color\" style=\"background: #0284c7; border: 1px solid #38bdf8;\"></div>\n";
    oss << "                <span>Process Node</span>\n";
    oss << "            </div>\n";
    oss << "            <div class=\"legend-item\">\n";
    oss << "                <div class=\"legend-color\" style=\"background: #7f1d1d; border: 1px solid #ef4444;\"></div>\n";
    oss << "                <span>Anomalous Process</span>\n";
    oss << "            </div>\n";
    oss << "            <div class=\"legend-item\">\n";
    oss << "                <div class=\"legend-color\" style=\"background: #111827; border: 1px solid #475569;\"></div>\n";
    ...
    oss << "                <h3 class=\"inspector-title\" id=\"inspect-title\">Node Inspector</h3>\n";
    oss << "                <div style=\"font-size: 0.8rem; color: var(--text-muted); margin-top: 0.25rem;\" id=\"inspect-subtitle\">Select a node to inspect parameters</div>\n";
    oss << "            </div>\n";
    oss << "            <div class=\"inspector-body\" id=\"inspect-body\">\n";
    oss << "                <div class=\"empty-state\">\n";
    oss << "                    <div class=\"empty-icon\">🔍</div>\n";
    oss << "                    <div>Click on any process or resource node in the graph to view active system metrics and details.</div>\n";
    oss << "                </div>\n";
    oss << "            </div>\n";
    oss << "        </div>\n";
    oss << "    </div>\n";
    ...
    oss << "        document.getElementById('header-symptom').innerText = symptomDesc;\n";
    oss << "        \n";
    oss << "        // Intro Typing Simulation\n";
    oss << "        const consoleLines = [\n";
    oss << "            \"🚀 [SysPilot Engine] Initializing causal tracing...\",\n";
    oss << "            \"🔍 [Symptom Node] Analyzing target process: \" + symptomDesc,\n";
    oss << "            \"📡 [Telemetry] Constructing hybrid eBPF & procfs directed multigraph...\",\n";
    oss << "            \"📊 [Traversal] Tracing back active edge chains (Reverse BFS)...\",\n";
    oss << "            \"⚠️ [Resource Contention] Saturated write rate & locked resources detected!\",\n";
    oss << "            \"💡 [Root Cause] Diagnosed process propagation error: close-on-exec (O_CLOEXEC) missing.\",\n";
    oss << "            \"✅ [Report] Dashboard generated successfully. Ready to inspect.\"\n";
    oss << "        ];\n";
    oss << "        let currentLine = 0;\n";
    oss << "        function typeConsole() {\n";
    oss << "            if (currentLine < consoleLines.length) {\n";
    oss << "                const consoleBox = document.getElementById('intro-console');\n";
    oss << "                const div = document.createElement('div');\n";
    oss << "                div.className = 'console-line';\n";
    oss << "                div.style.animationDelay = `${currentLine * 0.1}s`;\n";
    oss << "                div.innerHTML = `<span style=\"color: var(--cyan)\">&gt;</span> ${consoleLines[currentLine]}`;\n";
    ...
    oss << "            document.getElementById('inspect-title').innerText = data.name;\n";
    oss << "            document.getElementById('inspect-subtitle').innerText = `ID: ${data.id}`;\n";
    oss << "            \n";
    oss << "            let html = '';\n";
    oss << "            if (data.is_anomalous) {\n";
    oss << "                html += `\n";
    oss << "                    <div class=\"section-title\">Diagnostic Alert</div>\n";
    oss << "                    <div class=\"anomaly-alert\">\n";
    oss << "                        <strong>⚠️ Anomaly Detected</strong><br>\n";
    oss << "                        ${data.anomaly_reason || 'Process exhibiting anomalous resource or state metrics.'}\n";
    oss << "                    </div>\n";
    oss << "                `;\n";
    oss << "            }\n";
    oss << "            \n";
    oss << "            html += `<div class=\"section-title\">General Info</div>`;\n";
    oss << "            html += `<div class=\"metric-card\"><span class=\"metric-label\">Node Type</span><span class=\"metric-value\">${data.type.toUpperCase()}</span></div>`;\n";
    oss << "            \n";
    oss << "            if (data.type === 'process') {\n";
    oss << "                html += `\n";
    oss << "                    <div class=\"metric-card\"><span class=\"metric-label\">PID</span><span class=\"metric-value\">${data.pid}</span></div>\n";
    oss << "                    <div class=\"metric-card\"><span class=\"metric-label\">State</span><span class=\"metric-value\">${data.state}</span></div>\n";
    oss << "                    <div class=\"metric-card\"><span class=\"metric-label\">CPU Usage</span><span class=\"metric-value\">${data.cpu_usage_pct.toFixed(1)}%</span></div>\n";
    oss << "                `;\n";
    oss << "            }\n";
    oss << "            \n";
    oss << "            html += `\n";
    oss << "                <div class=\"section-title\">I/O Performance</div>\n";
    oss << "                <div class=\"metric-card\"><span class=\"metric-label\">Read Rate</span><span class=\"metric-value\">${data.read_rate_kb.toFixed(1)} KB/s</span></div>\n";
    oss << "                <div class=\"metric-card\"><span class=\"metric-label\">Write Rate</span><span class=\"metric-value\">${data.write_rate_kb.toFixed(1)} KB/s</span></div>\n";
    oss << "            `;\n";
    oss << "            \n";
    oss << "            document.getElementById('inspect-body').innerHTML = html;\n";
    oss << "        }\n";
    oss << "        \n";
    ...
    oss << "                        'height': '45px',\n";
    oss << "                        'padding': '10px',\n";
    oss << "                        'transition-property': 'background-color, border-color, border-width',\n";
    ...
    oss << "                        'transition-property': 'line-color, width, target-arrow-color',\n";
    oss << "                        'transition-duration': '0.2s'\n";
    oss << "                    }\n";
    oss << "                },\n";
    oss << "                {\n";
    ...
    oss << "            cy.elements().removeClass('highlighted').removeClass('dimmed');\n";
    oss << "            const path = node.successors().union(node.predecessors()).union(node);\n";
    oss << "            cy.elements().addClass('dimmed');\n";
    oss << "            path.removeClass('dimmed');\n";
    oss << "            node.addClass('highlighted');\n";
    oss << "            node.connectedEdges().removeClass('dimmed').addClass('highlighted');\n";
    oss << "        }\n";
    oss << "        \n";
    oss << "        function resetHighlight() {\n";
    oss << "            cy.elements().removeClass('highlighted').removeClass('dimmed');\n";
    oss << "            if (document.getElementById('d3-container').style.display === 'block') {\n";
    ...
    oss << "            }\n";
    oss << "            document.getElementById('inspect-title').innerText = 'Node Inspector';\n";
    oss << "            document.getElementById('inspect-subtitle').innerText = 'Select a node to inspect parameters';\n";
    ...
    oss << "            currentLayout = (currentLayout === 'dagre') ? 'cose' : 'dagre';\n";
    ...
    oss << "            });\n";
    oss << "            layout.run();\n";
    oss << "            document.getElementById('toggle-layout-btn').innerText = `Layout: ${currentLayout.toUpperCase()}`;\n";
    oss << "        }\n";
    oss << "        \n";
    oss << "        function initD3() {\n";
    oss << "            const container = document.getElementById('d3-container');\n";
    oss << "            container.innerHTML = '';\n";
    oss << "            const width = container.clientWidth || window.innerWidth - 380;\n";
    oss << "            const height = container.clientHeight || window.innerHeight - 80;\n";
    oss << "            \n";
    oss << "            const nodes = [];\n";
    oss << "            const links = [];\n";
    oss << "            \n";
    ...
    oss << "                        is_anomalous: el.data.is_anomalous || false,\n";
    ...
    oss << "                .data(['ACCESS', 'WRITES_TO', 'READS_FROM', 'BLOCKED_ON', 'SPAWNED_BY', 'CONTENDS_WITH'])\n";
    ...
    oss << "                .force('link', d3.forceLink(links).id(d => d.id).distance(180))\n";
    oss << "                .force('charge', d3.forceManyBody().strength(-500))\n";
    oss << "                .force('center', d3.forceCenter(width / 2, height / 2))\n";
    oss << "                .force('collision', d3.forceCollide().radius(100));\n";
    oss << "            \n";
    ...
    oss << "                .attr('stroke-width', d => d.type === 'BLOCKED_ON' ? 3 : 2)\n";
    oss << "                .attr('stroke', d => {\n";
    oss << "                    if (d.type === 'WRITES_TO') return '#f97316';\n";
    oss << "                    if (d.type === 'READS_FROM') return '#38bdf8';\n";
    oss << "                    if (d.type === 'BLOCKED_ON') return '#ef4444';\n";
    oss << "                    if (d.type === 'CONTENDS_WITH') return '#eab308';\n";
    oss << "                    return '#475569';\n";
    oss << "                })\n";
    oss << "                .attr('stroke-dasharray', d => d.type === 'CONTENDS_WITH' ? '5,5' : 'none')\n";
    ...
    oss << "                    return '#38bdf8';\n";
    oss << "                })\n";
    oss << "                .attr('stroke-width', d => d.is_anomalous ? 3 : 2);\n";
    oss << "            \n";
    ...
    oss << "                .text(d => {\n";
    oss << "                    if (d.name.length > 22) return d.name.substring(0, 19) + '...';\n";
    oss << "                    return d.name;\n";
    oss << "                });\n";
    oss << "            \n";
    ...
    oss << "                    .attr('y', d => (d.source.y + d.target.y) / 2 - 5);\n";
    oss << "                \n";
    oss << "                node\n";
    oss << "                    .attr('transform', d => `translate(${d.x},${d.y})`);\n";
    oss << "            });\n";
    oss << "            \n";
    oss << "            function dragstarted(event, d) {\n";
    oss << "                if (!event.active) simulation.alphaTarget(0.3).restart();\n";
    oss << "                d.fx = d.x;\n";
    oss << "                d.fy = d.y;\n";
    oss << "            }\n";
    oss << "            \n";
    ...
    oss << "                const connectedNodeIds = new Set([selectedNode.id]);\n";
    oss << "                links.forEach(l => {\n";
    oss << "                    if (l.source.id === selectedNode.id) connectedNodeIds.add(l.target.id);\n";
    oss << "                    if (l.target.id === selectedNode.id) connectedNodeIds.add(l.source.id);\n";
                // Add highlighting logic inside Javascript string
    oss << "                });\n";
    oss << "                node.selectAll('rect').attr('opacity', d => connectedNodeIds.has(d.id) ? 1.0 : 0.2);\n";
    oss << "                node.selectAll('text').attr('opacity', d => connectedNodeIds.has(d.id) ? 1.0 : 0.2);\n";
    oss << "                link.attr('opacity', l => (l.source.id === selectedNode.id || l.target.id === selectedNode.id) ? 1.0 : 0.15);\n";
    oss << "            }\n";
    oss << "        }\n";
    oss << "        \n";
    oss << "        function switchView(view) {\n";
    oss << "            document.querySelectorAll('.view-btn').forEach(btn => btn.classList.remove('active'));\n";
    oss << "            if (view === 'cytoscape') {\n";
    oss << "                document.getElementById('btn-cytoscape').classList.add('active');\n";
    oss << "                document.getElementById('cy').style.display = 'block';\n";
    oss << "                document.getElementById('d3-container').style.display = 'none';\n";
    oss << "                document.querySelector('.graph-controls').style.display = 'flex';\n";
    oss << "                cy.resize();\n";
    oss << "                cy.fit();\n";
    oss << "            } else {\n";
    oss << "                document.getElementById('btn-d3').classList.add('active');\n";
    oss << "                document.getElementById('cy').style.display = 'none';\n";
    oss << "                document.getElementById('d3-container').style.display = 'block';\n";
    oss << "                document.querySelector('.graph-controls').style.display = 'none';\n";
    oss << "                initD3();\n";
    oss << "            }\n";
    oss << "        }\n";
    oss << "    </script>\n";
